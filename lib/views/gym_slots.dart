...
class _GymSlotsState extends State<GymSlots> {
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(
                  isFull ? Icons.block : Icons.people,
                  color: Colors.white,
                  size: 14,
                ),
                const SizedBox(width: 4),
                Text(
                  isFull ? "Full" : _capacityLabel(capacity, booked),
                  style: const TextStyle(
                    color: Colors.white,
  ...
  Widget _buildDayChips(List<String> days, List<Color> gradient) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            Icon(Icons.calendar_month, size: 14, color: gradient[0]),
  ...
  Widget _buildBookButton(
    ...
    return GestureDetector(
      onTap: isBooking || isFull ? null : () => _handleBooking(index),
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          color: isFull ? Colors.grey.shade200 : null,
  ...
  Widget _buildSlotCard(int index) {
    final slot = slots[index];
    final int capacity = int.tryParse(slot['max_capacity'].toString()) ?? 0;
    final int booked = int.tryParse(slot['booked_count'].toString()) ?? 0;
    final bool isBooking = _bookingIndex == index;
    final bool isFull = booked >= capacity;
    final Color capacityColor = _capacityColor(capacity, booked);
    final List<Color> gradient = _getGradient(index);
    final List<String> days = _parseDays(slot['available_days'] ?? '');
    ...
    return Container(
      ...
      child: Column(
        children: [
          _buildCardHeader(slot, gradient, isFull, capacity, booked),
          Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                Row(
                  ...
                  children: [
                    Expanded(child: _buildDayChips(days, gradient)),
                    const SizedBox(width: 12),
                    _buildBookButton(index, isBooking, isFull, gradient),
                  ],
                ),
              ],
            ),
